#  Exercise 3: Build a Fabric Data Agent over trained model output
...
1. In the **fabric-<inject key="DeploymentID" enableCopy="false"/>** workspace, click on **+ New item (1)**, search for **Data agent (2)**, and select it from the results **(3)**.
    ...
    ![](./Images8/ex3-t6-s1.png)

    > **Note:** Fabric Data Agent is a preview feature. If it does not appear in your tenant, verify with your Fabric administrator that the Fabric IQ features are enabled, as configured in Exercise 1, Task 4.

1. Enter the name **churn-data-agent<inject key="DeploymentID" enableCopy="false"/> (1)** and click **Create (2)**.

    ![](./Images8/ex3-t6-s2.png)

1. In the Data Agent editor, click **Add data source (1)**.

    ![](./Images8/ex3-t6-s3.png)
...
1. From the OneLake catalog, select **Lakehouse_<inject key="DeploymentID" enableCopy="false"/> (1)**, then choose the **churn_predictions (2)** table and click **Add (3)**.
    ...
    ![](./Images8/ex3-t6-s4.png)

1. Confirm the **churn_predictions** table appears under **Data sources** in the left pane.

    ![](./Images8/ex3-t6-s5.png)

1. Select the **Instructions (1)** tab and enter the following:
    ...
    ![](./Images8/ex3-t6-s6.png)

1. Select the **Example questions (1)** tab and click **+ Add example (2)**.

    ![](./Images8/ex3-t6-s7.png)

1. Enter the following first example question:

    ```
    How many customers are predicted to churn?
    ```

1. Click **+ Add example** again and enter the following second example question:

    ```
    Which customers have the highest churn probability?
    ```

    ![](./Images8/ex3-t6-s9.png)

1. Click **Publish (1)** to publish the agent.

    ![](./Images8/ex3-t6-s10.png)
...
1. Review the agent's answer. Click **Show generated query (1)** to confirm the SQL it ran against the **churn_predictions** table matches your expectations.

    ![](./Images8/ex3-t6-s12.png)
...
- Created and published the **churn-data-agent<inject key="DeploymentID" enableCopy="false"/>** Fabric Data Agent over the predictions table and validated it with three natural-language queries about predicted customer churn.
